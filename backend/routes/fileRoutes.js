...
router.get('/users/:userId', async (req, res) => {   //   (/files/users/:userId)
  try {
    ...
    if (filesResponse.length === 0) {
      return res.status(404).send('No files found matching the specified types for this user.');
    }
...
router.get('/reports/:reportId/users/:userId', async (req, res) => {   //   (/files/users/:userId)
  try {
    // Extract userId from the request parameters
    ...
    // Fetch reports for the specified user
    const reports = await dbconnect.getAllReportsWhere({ reportId: reportId, userId: userId });
...
router.put('/:fileName/reports/:reportId/users/:userId', checkReportExists, async (req, res) => {
...
//////////////// DELETE
router.delete('/:fileName/reports/:reportId/users/:userId', async (req, res) => {
  const { fileName, reportId, userId } = req.params;
  ...
  try {
    ...
    const fileIndex = report.files.findIndex(file => file.fileName === fileName);
    if (fileIndex === -1) {
      return res.status(404).json({ success: false, message: 'File not found.' });
    }
    ...
    res.json({ success: true, message: 'File deleted successfully.', report:report });
  } catch (error) {
